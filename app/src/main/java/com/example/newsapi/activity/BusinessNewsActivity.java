...
public class BusinessNewsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rvTech.addOnItemTouchListener(new RecyclerItemListener(BusinessNewsActivity.this, rvTech, new RecyclerItemListener.ClickListener() {
            @Override
            public void onClick(View view, int position) {
                News news = adapter.getItem(position);
                Intent title_Intent = new Intent(BusinessNewsActivity.this, WebviewNewsActivity.class);
    ...
    private void loadFirstPage() {
        if (CoreApp.checkConnection((BusinessNewsActivity.this))) {
            ApiResponse request = ApiClient.getApiService();

            Call<NewsResources> call = request.getNewsByCategory(COUNTRY, CATEGORY_BUSINESS, API_KEY,10,currentPage);
            call.enqueue(new Callback<NewsResources>() {
                ...
                public void onFailure(Call<NewsResources> call, Throwable t) {
                    progressTech.setVisibility(View.GONE);
                    Toast.makeText(BusinessNewsActivity.this, "Something went wrong...", Toast.LENGTH_SHORT).show();
                }
            });
        } else {
            Toast.makeText(BusinessNewsActivity.this, "Internet connection not Available", Toast.LENGTH_SHORT).show();
            progressTech.setVisibility(View.GONE);
        }
    }
