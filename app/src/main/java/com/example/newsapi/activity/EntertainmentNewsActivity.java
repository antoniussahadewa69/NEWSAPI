...
import static com.example.newsapi.model.Contants.CATEGORY_BUSINESS;
import static com.example.newsapi.model.Contants.CATEGORY_ENTERTAINMENT;
import static com.example.newsapi.model.Contants.CATEGORY_SPORTS;
...
public class EntertainmentNewsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rvTech.addOnItemTouchListener(new RecyclerItemListener(EntertainmentNewsActivity.this, rvTech, new RecyclerItemListener.ClickListener() {
            @Override
            public void onClick(View view, int position) {
                News news = adapter.getItem(position);
                Intent title_Intent = new Intent(EntertainmentNewsActivity.this, WebviewNewsActivity.class);
    ...
    private void loadFirstPage() {
        if (CoreApp.checkConnection((EntertainmentNewsActivity.this))) {
            ApiResponse request = ApiClient.getApiService();

            Call<NewsResources> call = request.getNewsByCategory(COUNTRY, CATEGORY_ENTERTAINMENT, API_KEY,10,currentPage);
            call.enqueue(new Callback<NewsResources>() {
                ...
                public void onFailure(Call<NewsResources> call, Throwable t) {
                    progressTech.setVisibility(View.GONE);
                    Toast.makeText(EntertainmentNewsActivity.this, "Something went wrong...", Toast.LENGTH_SHORT).show();
                }
            });
        } else {
            Toast.makeText(EntertainmentNewsActivity.this, "Internet connection not Available", Toast.LENGTH_SHORT).show();
            progressTech.setVisibility(View.GONE);
        }
    }
