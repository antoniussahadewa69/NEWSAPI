...
import static com.example.newsapi.model.Contants.CATEGORY_HEALTH;
...
public class HealthNewsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rvTech.addOnItemTouchListener(new RecyclerItemListener(HealthNewsActivity.this, rvTech, new RecyclerItemListener.ClickListener() {
            @Override
            public void onClick(View view, int position) {
                News news = results.get(position);
                Intent title_Intent = new Intent(HealthNewsActivity.this, WebviewNewsActivity.class);
    ...
    private void loadFirstPage() {
        if (CoreApp.checkConnection((HealthNewsActivity.this))) {
            ApiResponse request = ApiClient.getApiService();

            Call<NewsResources> call = request.getNewsByCategory(COUNTRY, CATEGORY_HEALTH, API_KEY,10,currentPage);
            call.enqueue(new Callback<NewsResources>() {
                ...
                public void onFailure(Call<NewsResources> call, Throwable t) {
                    progressTech.setVisibility(View.GONE);
                    Toast.makeText(HealthNewsActivity.this, "Something went wrong...", Toast.LENGTH_SHORT).show();
                }
            });
        } else {
            Toast.makeText(HealthNewsActivity.this, "Internet connection not Available", Toast.LENGTH_SHORT).show();
            progressTech.setVisibility(View.GONE);
        }
    }
