...
import com.example.newsapi.model.Contants;
...
import static com.example.newsapi.model.Contants.CATEGORY_SCIENCE;
...
public class ScienceNewsActivity extends AppCompatActivity {
    ...
    private void loadFirstPage() {
        if (CoreApp.checkConnection((ScienceNewsActivity.this))) {
            ApiResponse request = ApiClient.getApiService();

            Call<NewsResources> call = request.getNewsByCategory(COUNTRY, CATEGORY_SCIENCE, API_KEY,10,currentPage);
            call.enqueue(new Callback<NewsResources>() {
                ...
                public void onFailure(Call<NewsResources> call, Throwable t) {
                    progressTech.setVisibility(View.GONE);
                    Toast.makeText(ScienceNewsActivity.this, "Something went wrong...", Toast.LENGTH_SHORT).show();
                }
            });
        } else {
            Toast.makeText(ScienceNewsActivity.this, "Internet connection not Available", Toast.LENGTH_SHORT).show();
            progressTech.setVisibility(View.GONE);
        }
    }
