...
public class SportsNewsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rvTech.addOnItemTouchListener(new RecyclerItemListener(SportsNewsActivity.this, rvTech, new RecyclerItemListener.ClickListener() {
            @Override
            public void onClick(View view, int position) {
                News news = results.get(position);
                Intent title_Intent = new Intent(SportsNewsActivity.this, WebviewNewsActivity.class);
    ...
    private void loadFirstPage() {
        if (CoreApp.checkConnection((SportsNewsActivity.this))) {
            ApiResponse request = ApiClient.getApiService();

            Call<NewsResources> call = request.getNewsByCategory(COUNTRY, CATEGORY_SPORTS, API_KEY,10,currentPage);
            call.enqueue(new Callback<NewsResources>() {
                ...
                public void onFailure(Call<NewsResources> call, Throwable t) {
                    progressTech.setVisibility(View.GONE);
                    Toast.makeText(SportsNewsActivity.this, "Something went wrong...", Toast.LENGTH_SHORT).show();
                }
            });
        } else {
            Toast.makeText(SportsNewsActivity.this, "Internet connection not Available", Toast.LENGTH_SHORT).show();
            progressTech.setVisibility(View.GONE);
        }
    }
